...
* **Comprovantes na Cloudinary (concluído):** com `EXPO_PUBLIC_API_URL` configurada, o app envia o comprovante pela assinatura do `snake-server` (Render) direto para a Cloudinary; sem ela, cai no Supabase Storage legado. Os dois convivem enquanto houver comprovante antigo no Storage. Detalhes em `docs/BACKEND.md`. [#80]
* **Três repositórios, um contrato:** `snake-thai` (app e dono do banco), `snake-server` (rotas `/v1/*` e worker de mídia) e `snake-web` (web do aluno). Todo nome que cruza repositório está em `docs/CONTRATO.md`, que só este repositório edita, com aprovação do usuário. A fila de trabalho viva é o `ROADMAP-thai.md`.
